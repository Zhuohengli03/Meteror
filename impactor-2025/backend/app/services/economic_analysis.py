...
async def fetch_nearby_cities(
    ...
    # Try GeoNames API first (only if username is configured and not "demo")
    if geonames_username and geonames_username != "demo" and geonames_username.strip():
        try:
            logger.info(f"Attempting to fetch cities from GeoNames API (username: {geonames_username})")
            client = GeoNamesClient(username=geonames_username)
            
            # Add timeout wrapper for the entire API call
            import asyncio
            geonames_cities = await asyncio.wait_for(
                client.get_nearby_cities(
                    latitude=latitude,
                    longitude=longitude,
                    radius_km=radius_km,
                    max_rows=500,
            ...
            if cities:
                logger.info(f"Successfully fetched {len(cities)} cities from GeoNames API")
                return cities
            else:
                logger.info("GeoNames API returned no cities, using fallback database")
            
        except asyncio.TimeoutError:
            logger.warning(f"GeoNames API timeout after 8 seconds, falling back to static database")
        except Exception as e:
            logger.warning(f"GeoNames API failed ({type(e).__name__}: {e}), falling back to static database")
    else:
        logger.info("GeoNames username not configured, using fallback city database")
    
    # Fallback to static database
    logger.info(f"Using fallback city database with {len(FALLBACK_CITIES)} cities")
